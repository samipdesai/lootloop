...
| 51  | Pre-production security & data audit            | `get_advisors` security + perf lints clean; RLS verified on every table incl. cross-family isolation; `service_role` confirmed absent from client bundles; PITR/backups enabled; secrets rotated                                                         | #49                | 3h  |
...
| 61  | Error & crash monitoring                        | Sentry (or equivalent) wired into web (Next.js) + mobile (RN); dSYM / source-map upload in release builds; alerting on prod errors                                                                                                                       | #49                | 3h  |
